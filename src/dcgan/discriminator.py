...
class FiveKernelSixLayerDiscriminator(nn.Module):
    def __init__(self, n_gpu, fm_depth, img_nc):
        super(FiveKernelSixLayerDiscriminator, self).__init__()
        self.ngpu = n_gpu
        self.fm_depth = fm_depth
        self.img_nc = img_nc
        self.main = nn.Sequential(
            ...
            # state size. (ndf*8) x 4 x 4
            # output depth -> fm_depth = 512
            nn.Conv2d(self.fm_depth * 8, self.fm_depth * 12, 5, 2, 1, bias=False),
            nn.BatchNorm2d(self.fm_depth * 12),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (ndf*8) x 4 x 4
            # output depth -> fm_depth = 512
            nn.Conv2d(self.fm_depth * 12, 1, 5, 1, 0, bias=False),
            nn.Sigmoid()
        )
